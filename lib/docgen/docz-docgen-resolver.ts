...
const { default: resolveToModule } = require('react-docgen/dist/utils/resolveToModule')
...
export const createStyledComponentResolvers = () => {
  ...
  const isStyledExpression = (tagPath: Todo, t: Todo) =>
    (t.CallExpression.check(tagPath.node) && tagPath.get('callee').node.name === 'styled') ||
    (t.MemberExpression.check(tagPath.node) && tagPath.get('object').node.name === 'styled')
...
export const resolver = (ast: Todo, recast: Todo) => [
  ...
  ...createStyledComponentResolvers().findAllExportedStyledComponents(ast, recast),
]
