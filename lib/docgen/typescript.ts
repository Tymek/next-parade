...
function getPropsOnCache(): any {
  const cache = readCacheFile()
  if (_.isEmpty(cache)) {
    console.warn('No cache was found with your props definitions')
    console.warn("We'll parse your components to get props from them")
    console.warn('Depending on your components, this could take while...')
...
const parseFiles = (files: string[], tsconfig: string) => {
  const opts = {
    propFilter(prop: any): boolean {
      // const customPropFilter = config.docgenConfig.propFilter
      const propFilter = defaultPropFilter
      // const propFilter =
...
export const tsParser = (
  ...
  const next = parseFiles(filesToLoad, tsconfig)
